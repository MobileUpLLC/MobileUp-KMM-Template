...
/**
 * Класс `RootHolder` используется для хранения и инициализации корневого компонента приложения.
 * Он управляет настройками и контекстом компонента, который используется в приложении.
 * Этот класс инициализирует все зависимости, включая конфигурацию и жизненный цикл
