...
// Code from Decompose sample - https://github.com/arkivanov/Decompose/blob/master/sample/app-ios/app-ios/StackView.swift

struct StackView<T: AnyObject, Content: View>: View {
    
    @ObservedObject var stackState: ObservableState<ChildStack<AnyObject, T>>

    var getTitle: (T) -> String = { _ in "" }
                ...
                childContent(stack.first!.instance!)
                    .navigationDestination(for: Child<AnyObject, T>.self) {
                        childContent($0.instance!)
                            .navigationTitle(getTitle($0.instance!))
                    }
            }
        } else {
            StackInteropView(
